...
class UserAPIController extends Controller
{
    ...
    function loginSocial(Request $request)
    {
        try {
            ...
            if ($user==null && ($request->email!=null ||$request->phone_number!=null)) {
                $user = new User;
                ...
                $user->save();
                // $defaultRoles = $this->roleRepository->findByField('provider');
                // $defaultRoles = $defaultRoles->pluck('name')->toArray();
                $user->assignRole('provider');

                try {
                    $upload = $this->uploadRepository->create(['uuid' => $request->token]);
                    $upload->addMediaFromUrl($request->avatar_original)
                        ->withCustomProperties(['uuid' => $request->token])
                        ->toMediaCollection('avatar');

                    $cacheUpload = $this->uploadRepository->getByUuid($request->token);
            ...
            if($user==null){
                Flash::error('Please add Email or phone number');
            } 
            return $this->sendResponse($user, 'User retrieved successfully');
        } catch (ValidationException $e) {
            return $this->sendError(array_values($e->errors()));
