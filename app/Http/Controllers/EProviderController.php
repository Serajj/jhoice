...
class EProviderController extends Controller
{
    ...
    public function __construct(EProviderRepository $eProviderRepo,RoleRepository $roleRepo, CustomFieldRepository $customFieldRepo, UploadRepository $uploadRepo
    ...
    public function edit(int $id)
    {
        ...
        return view('e_providers.edit')->with('eProvider', $eProvider)->with("customFields", isset($html) ? $html : false)->with("eProviderType", $eProviderType)->with("user", $user)->with("usersSelected", $usersSelected)->with("address", $address)->with("addressesSelected", $addressesSelected)->with("tax", $tax)->with("taxesSelected", $taxesSelected)->with("role", $role)->with("rolesSelected", $rolesSelected);
    }
    ...
    public function update(int $id, UpdateEProviderRequest $request)
    {
        ...
        try {
			...
			print_r($input);
			exit;*/
            //Old method - If need to select multiple user or employee uncomment this and comment below code
